package com.citelic.utility.npc;
...
public final class NPCSpawns {
	...
	private static final void addNPCSpawn(int npcId, int regionId, Tile tile,
	...
	public static boolean addSpawn(String username, int id, Tile tile)
		...
		synchronized (lock) {
			...
			writer.flush();
			writer.close();
			Engine.spawnNPC(id, tile, -1, true);
			return true;
		}
	...
	public static final void loadNPCSpawns(int regionId) {
		...
		try {
			...
			while (buffer.hasRemaining()) {
				...
				Engine.spawnNPC(npcId, new Tile(x, y, plane), mapAreaNameHash,
		...
		try {
			...
			while (true) {
				...
				if (splitedLine2.length != 3 && splitedLine2.length != 5)
					throw new RuntimeException("Invalid NPC Spawn line: "
							+ line);
				Tile tile = new Tile(Integer.parseInt(splitedLine2[0]),
