...
public class ItemConstants {
	...
	if (!item.getDefinitions().isWearItem())
		...
		if (skill == Skills.DUNGEONEERING || skill == Skills.CONSTRUCTION) {
		    continue;
		}
		if (player.getSkills().getLevel(skill) < (skill == Skills.DUNGEONEERING ? 120 : 99)) {
	...
	for (int itemIndex = item.getId() - ITEMS_RANGE; itemIndex < item.getId() + ITEMS_RANGE; itemIndex++) {
		...
		player.getInventory().removeItemMoneyPouch(new Item(995, (int) price));
	    else {
    ...
    public static boolean isInsideDungItem(int itemId) {
