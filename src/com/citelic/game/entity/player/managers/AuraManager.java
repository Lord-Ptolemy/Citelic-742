...
public class AuraManager implements Serializable {
		...
		if (toId != -1) {
			...
			if (player.getControllerManager().getController() instanceof DuelArena) {
				player.getPackets().sendGameMessage(
						"You cannot activate your aura while dueling!.");
				return;
			}
			int tier = getTier(item.getId());
			activation = Utilities.currentTimeMillis()
	...
	public void checkSuccefulHits(int damage) {
		if (!isActivated() || Engine.isPvpArea(player))
			return;
	...
	public void sendAuraRemainingTime() {
			...
			if (Utilities.currentTimeMillis() <= cooldown) {
				player.getPackets().sendGameMessage(
						"Currently recharging. <col=ff0000>"
								+ getFormatedTime((cooldown - Utilities
		...
		player.getPackets().sendGameMessage(
				"Currently active. <col=00ff00>"
						+ getFormatedTime((activation - Utilities
