...
public final class MusicsManager implements Serializable {
	...
	public void checkMusic(int requestMusicId) {
		if (playListOn
				|| settedMusic
				&& playingMusicDelay + (180000) >= Utilities
						.currentTimeMillis())
			return;
		settedMusic = false;
	...
	public boolean musicEnded() {
		return playingMusic != -2
				&& playingMusicDelay + (180000) < Utilities.currentTimeMillis();
	}
		...
		if (playListOn && playList.size() > 0) {
			if (shuffleOn)
				playingMusic = playList
						.get(Utilities.getRandom(playList.size() - 1));
			else {
			...
			playingMusic = unlockedMusics.get(Utilities
					.getRandom(unlockedMusics.size() - 1));
		playMusic(playingMusic);
	}
	...
	public void reset() {
		...
		player.getMusicsManager().checkMusic(
				Engine.getRegion(player.getRegionId()).getMusicId());
	}
	...
	public void setPlayer(Player player) {
		this.player = player;
		playingMusic = Engine.getRegion(player.getRegionId()).getMusicId();
	}
