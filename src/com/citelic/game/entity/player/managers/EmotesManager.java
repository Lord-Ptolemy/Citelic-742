...
public final class EmotesManager implements Serializable {
	...
	public void useBookEmote(int slotId) {
		...
		if (slotId == 2) {// Yes
			...
			case 24709: // Veteran cape 10years
				if (!Engine.isTileFree(player.getZ(), player.getX(),
						player.getY(), 3)) {
					player.getPackets()
							.sendGameMessage(
			...
			case 20767: // Max cape
				...
				Tile spawnTile = new Tile(new Tile(player.getX() + 1,
						player.getY(), player.getZ()));
				if (!Engine.isTileFree(spawnTile.getZ(), spawnTile.getX(),
						spawnTile.getY(), size)) {
					spawnTile = null;
					int[][] dirs = Utilities.getCoordOffsetsNear(size);
					for (int dir = 0; dir < dirs[0].length; dir++) {
						final Tile tile = new Tile(new Tile(player.getX()
								+ dirs[0][dir], player.getY() + dirs[1][dir],
								player.getZ()));
			...
			player.lock(10);
			nextEmoteEnd = Utilities.currentTimeMillis() + (10 * 600);
			EngineTaskManager.schedule(new EngineTask() {
		...
		} else if (slotId == 47) {// Around the world in Eggy days
		...
		} else if (slotId == 50) {// Puppet SlayerMasterD
			...
			player.lock(6);
			nextEmoteEnd = Utilities.currentTimeMillis() + (6 * 600);
			EngineTaskManager.schedule(new EngineTask() {
				@Override
				public void run() {
					if (step == 0) {
			...
			if (!Engine.isTileFree(player.getZ(), player.getX(), player.getY(),
					3)) {
				player.getPackets().sendGameMessage(
			...
			final NPC n = new NPC(14388, new Tile(player.getX(),
					player.getY() + 2, player.getZ()), 0, false);
			n.setLocation(n);
	...
	public boolean isDoingEmote() {
		return nextEmoteEnd >= Utilities.currentTimeMillis();
	}
