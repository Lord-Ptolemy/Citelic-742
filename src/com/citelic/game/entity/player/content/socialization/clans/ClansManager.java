package com.citelic.game.entity.player.content.socialization.clans;
...
public class ClansManager {
	...
	public static void viewClanmateDetails(Player player, ClanMember member) {
		...
		player.getPackets().sendGlobalString(347,
				Utilities.formatPlayerNameForDisplay(member.getUsername()));
	...
	public void kickPlayer(ClanMember mate) {
		synchronized (cachedClans) {
			synchronized (this) {
				...
				Player player = Engine.getPlayer(mate.getUsername());
				if (player != null) {
					...
					if (mate.getRank() == Clan.LEADER) {
						...
						if (newLeader != null) {
							...
							sendGlobalMessage("<col=7E2217>"
									+ Utilities
											.formatPlayerNameForDisplay(newLeader
													.getUsername())
		...
		synchronized (manager) {
			if (rank != null && member.getRank() == Clan.LEADER) {
				...
				manager.sendGlobalMessage("<col=7E2217>"
						+ Utilities.formatPlayerNameForDisplay(newLeader
								.getUsername())
						+ " has been appointed as new leader!");
			}
			if (rank != null) {
	...
	public void generateClanSettingsDataBlock() {
		...
		for (ClanMember member : clan.getMembers()) {
			// stream.writeLong(
			stream.writeString(Utilities.formatPlayerNameForDisplay(member
					.getUsername()));
	...
	public static void createClan(Player player, String clanName) {
		clanName = Utilities.formatPlayerNameForDisplay(clanName);
		if (player.getClanManager() != null)
			return;
		synchronized (cachedClans) {
			...
			if (!created) { // not loaded
				...
				synchronized (manager) {
					if (guest) {
						...
						if (bannedSince != null) {
							if (bannedSince + 3600000 > Utilities
									.currentTimeMillis()) {
								player.getPackets()
										.sendGameMessage(
	...
	public static void banPlayer(Player player, String name) {
		name = Utilities.formatPlayerNameForDisplay(name);
		ClansManager manager = player.getClanManager();
		if (manager == null) {
			player.getPackets().sendGameMessage("You're  not in a clan.");
			return;
