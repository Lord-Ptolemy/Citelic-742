...
public class XPLamp {
	...
	public static final int[] SELECTABLE_XP_LAMPS_TYPES = new int[] {
			XPLamp.LAMP_SMALL, XPLamp.LAMP_MEDIUM, XPLamp.LAMP_BIG,
			XPLamp.LAMP_HUGE };
	...
	private static double getExp(int skillLevel, int lampType) {
		double xp;
		if (skillLevel <= XPLamp.BASE_LAMPS_XP.length) {
			xp = XPLamp.BASE_LAMPS_XP[skillLevel - 1];
		} else {
			xp = XPLamp.BASE_LAMPS_XP[XPLamp.BASE_LAMPS_XP.length - 1];
		}
	...
	public static boolean isOtherLamp(int id) {
		for (int i = 0; i < XPLamp.OTHER_LAMPS.length; i++) {
			if (XPLamp.OTHER_LAMPS[i] == id)
				return true;
		}
		return false;
	}
	...
	public static boolean isSelectable(int id) {
		for (int i = 0; i < XPLamp.SELECTABLE_XP_LAMPS.length; i++) {
			if (XPLamp.SELECTABLE_XP_LAMPS[i] == id)
				return true;
		}
		return false;
	}
	...
	public static boolean isSkillLamp(int id) {
		for (int skillId = 0; skillId < XPLamp.SKILL_LAMPS.length; skillId++) {
			for (int i = 0; i < XPLamp.SKILL_LAMPS[skillId].length; i++) {
				if (XPLamp.SKILL_LAMPS[skillId][i] == id)
					return true;
			}
		}
		return false;
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void run(int interfaceId, int componentId) {
				if (componentId >= 13 && componentId <= 37) {
					int skill = XPLamp.DIALOGUE_INTERFACE_C2S[componentId - 13];
					...
					double exp = player.getSkills().addXpLamp(
							...
							XPLamp.getExp(
									player.getSkills().getLevelForXp(
											selectedSkill),
									XPLamp.selectableLampType(id)));
					player.getInterfaceManager().closeScreenInterface();
					sendDialogue(
							"<col=0000ff>Your wish has been granted!</col>",
							"You have been awarded "
									+ Utilities.getFormattedNumber(exp, ',')
									+ " XP in "
	...
	private static void openSkillDialog(Player player, final int slot,
			final int id) {
		final int type = XPLamp.skillLampType(id);
		final int skillId = XPLamp.skillLampSkillId(id);
	...
	public static void processLampClick(Player player, int slot, int id) {
		...
		if (XPLamp.isSelectable(id)) {
			XPLamp.openSelectableDialog(player, slot, id);
		} else if (XPLamp.isSkillLamp(id)) {
			XPLamp.openSkillDialog(player, slot, id);
		} else if (XPLamp.isOtherLamp(id)) {
			// for future
		}
	}
	...
	private static int selectableLampType(int id) {
		for (int i = 0; i < XPLamp.SELECTABLE_XP_LAMPS.length; i++) {
			if (XPLamp.SELECTABLE_XP_LAMPS[i] == id)
				return XPLamp.SELECTABLE_XP_LAMPS_TYPES[i];
		}
		return -1;
	}
	...
	private static int skillLampType(int id) {
		for (int skillId = 0; skillId < XPLamp.SKILL_LAMPS.length; skillId++) {
			for (int i = 0; i < XPLamp.SKILL_LAMPS[skillId].length; i++) {
				if (XPLamp.SKILL_LAMPS[skillId][i] == id)
					return XPLamp.SKILL_LAMPS_TYPES[skillId][i];
			}
		}
		return -1;
	}
