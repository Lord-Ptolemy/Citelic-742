...
public class ClueScrolls {
	...
	private enum Maps {
		Map1(337, 2971, 3414,
				...
				352, 2906, 3295,
				"Rite next to a guild known for people with skilled hands!"), Map11(
				353, 2616, 3077,
	...
	private enum Riddles {
		...
		Riddle4(14, new int[] { 2987, 3123, 3001, 3109 }, new String[] {
				...
				"A simple emote is needed", "An emote known as dance!", "", "",
				"", "" }), Riddle5(6, new int[] { 2884, 3449, 2898, 3438 },
				new String[] { "This reward will require a bit",
	...
	static Item[] EasyRewards = { new Item(88), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(7319),
			new Item(7321), new Item(7323), new Item(7325), new Item(7327),
			new Item(7332), new Item(7334), new Item(7336), new Item(7338),
			new Item(7340), new Item(7342), new Item(74),
			...
			new Item(7360), new Item(7362), new Item(7364), new Item(7366),
			new Item(7368), new Item(7370), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(7372),
			new Item(7374), new Item(7376), new Item(7378), new Item(7380),
			new Item(7382), new Item(7384), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(74),
			...
			new Item(10306), new Item(10308), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(74),
			new Item(995, 100000), new Item(995, 100000), new Item(10310),
			new Item(10312), new Item(10314), new Item(10368), new Item(10370),
			...
			new Item(10392), new Item(10394), new Item(10396), new Item(10398),
			new Item(10400) };

	static Item[] ThirdAge = { new Item(10330), new Item(10332),
			new Item(10334), new Item(19308), new Item(19320), new Item(19311),
			new Item(19314), new Item(19317), new Item(10336), new Item(10338),
			new Item(10340), new Item(10342), new Item(10344), new Item(10346),
			new Item(10348), new Item(10350), new Item(10352) };

	public static boolean completedRiddle(Player player, int emoteid) {
		Scrolls scroll = hasClue(player);
		if (scroll != null) {
			if (hasRiddleClue(player, scroll.id) != null) {
				Riddles riddleclue = hasRiddleClue(player, scroll.id);
				Tile lastloc = player.getLastWorldTile();
				if (lastloc.getX() >= riddleclue.locations[0]
					...
					if (emoteid == riddleclue.emoteid) {
						...
						player.getInventory().deleteItem(scroll.id, 1);
	...
	public static boolean digSpot(Player player) {
		Scrolls scroll = hasClue(player);
		if (scroll != null) {
			if (hasMapClue(player, scroll.id) != null) {
				Maps mapclue = hasMapClue(player, scroll.id);
				Tile lastloc = player.getLastWorldTile();
	...
	public static void giveReward(Player player) {
		int random = Utilities.random(999) + 1;
		if (player.getClueReward() == 1) {
			random += 100;
		} else if (player.getClueReward() == 2) {
			random += 250;
			player.setClueReward(player.getClueReward() + 1);
		} else if (player.getClueReward() == 3) {
			random += 450;
		}
		if (random > 500) {
			Item[] rewards;
			if (Utilities.random(200) > 185 && Utilities.random(400) > 200) {
				rewards = new Item[] {
						EasyRewards[Utilities.random(EasyRewards.length)],
						EasyRewards[Utilities.random(EasyRewards.length)],
						EasyRewards[Utilities.random(EasyRewards.length)],
						ThirdAge[Utilities.random(ThirdAge.length)] };
			} else {
				rewards = new Item[] {
						EasyRewards[Utilities.random(EasyRewards.length)],
						EasyRewards[Utilities.random(EasyRewards.length)],
			...
			for (Item item : rewards) {
				...
				player.getPackets().sendGameMessage(
						"You open the casket, and find....");
				player.getInterfaceManager().sendInterface(364);
				player.getPackets().sendInterSetItemsOptionsScript(364, 4, 90,
						3, 4, "Examine");
			...
			player.setClueReward(0);
		} else {
			player.getPackets().sendGameMessage(
					"You found another clue scroll inside the casket!");
			player.getInventory().deleteItem(2717, 1);
			player.getInventory().addItem(
					ScrollIds[Utilities.random(ScrollIds.length)], 1);
			if (player.getClueReward() == 0) {
				player.setClueReward(player.getClueReward() + 1);
			}
		}
	}

	public static Scrolls hasClue(Player player) {
		for (Scrolls scroll : Scrolls.values()) {
			if (player.getInventory().containsOneItem(scroll.id)) {
				return scroll;
			}
		}
		return null;
	...
	public static Maps hasMapClue(Player player, int scrollid) {
		for (Scrolls scroll : Scrolls.values()) {
			if (scroll.getHiding() == null)
				continue;
			else {
	...
	public static ObjectMaps hasObjectMapClue(Player player, int scrollid) {
		for (Scrolls scroll : Scrolls.values()) {
			if (scroll.getLocationMap() == null)
				continue;
			else {
	...
	public static Riddles hasRiddleClue(Player player, int scrollid) {
		for (Scrolls scroll : Scrolls.values()) {
			if (scroll.getRiddle() == null)
				continue;
			else {
	...
	public static boolean objectSpot(Player player, GameObject object) {
		Scrolls scroll = hasClue(player);
		if (scroll != null) {
			if (hasObjectMapClue(player, scroll.id) != null) {
				ObjectMaps mapclue = hasObjectMapClue(player, scroll.id);
				player.getLastWorldTile();
				if (object.getX() == mapclue.objectx
						&& object.getY() == mapclue.objecty
						&& object.getId() == mapclue.objectid) {
					player.getPackets()
							.sendGameMessage(
									"You have succesfully completed the riddle and have been awarded a chest!");
					player.getInventory().deleteItem(scroll.id, 1);
	...
	public static void showMap(Player player, Maps map) {
		player.getInterfaceManager().sendInterface(map.interfaceId);
		player.getPackets().sendGameMessage(map.chat);
	}

	public static void showObjectMap(Player player, ObjectMaps objmap) {
		player.getInterfaceManager().sendInterface(objmap.interid);
		player.getPackets().sendGameMessage(objmap.hint);
	}

	public static void showRiddle(Player player, Riddles riddle) {
		player.getInterfaceManager().sendInterface(345);
		player.getPackets().sendIComponentText(345, 1, riddle.riddles[0]);
		player.getPackets().sendIComponentText(345, 2, riddle.riddles[1]);
		player.getPackets().sendIComponentText(345, 3, riddle.riddles[2]);
		player.getPackets().sendIComponentText(345, 4, riddle.riddles[3]);
		player.getPackets().sendIComponentText(345, 5, riddle.riddles[4]);
		player.getPackets().sendIComponentText(345, 6, riddle.riddles[5]);
		player.getPackets().sendIComponentText(345, 7, riddle.riddles[6]);
		player.getPackets().sendIComponentText(345, 8, riddle.riddles[7]);
	}
}