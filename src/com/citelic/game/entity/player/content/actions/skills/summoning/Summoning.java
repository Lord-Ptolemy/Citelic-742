package com.citelic.game.entity.player.content.actions.skills.summoning;
...
public class Summoning implements Serializable {
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		String loc = "com.citelic.game.entity.npc.impl.familiar.";
		try {
			Familiar fam = (Familiar) Class
					...
					.getConstructor(
							new Class[] { Player.class, Pouches.class,
									Tile.class, Integer.TYPE, Boolean.TYPE })
					.newInstance(
	...
	public static void createPouch(final Player player, int itemId, int amount) {
		...
		for (i = 0; i < amount; i++) {
			...
			player.getPackets().sendObjectAnimation(
					new GameObject(28716, 10, 1, 2209, 5344, 0),
					new Animation(8509));
		}
		if (i == 1) {
			player.getPackets().sendGameMessage(
