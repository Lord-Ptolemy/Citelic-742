...
public class HarvestAction extends Action {
	...
	public HarvestAction(Player player, Patch patch) {
		...
		if (tree != null) {
			...
			xp = allotment.getXp();
			if (patch.yield < 1) {
				patch.yield = Utilities.random(3, 15);
			}
			produce = allotment.getProduceId();
			requiredItem = PatchConstants.SPADE;
			if (patch.composted != 0) {
				patch.yield += Utilities.random(0, 2);
			}
		} else if (hop != null) {
			xp = hop.getXp();
			if (patch.yield < 1) {
				patch.yield = Utilities.random(3, 35);
			}
			...
			if (patch.yield < 1) {
				patch.yield = Utilities.random(3, 8);
			}
			requiredItem = -1;
			if (patch.composted != 0) {
				patch.yield += Utilities.random(0, 2);
	...
	public boolean checkAll(Player player) {
		if (tree != null) {
			...
			if (animation == -1) {
				player.getPackets()
						.sendGameMessage(
								"You need a hatchet that you are able to use at your Woodcutting level to chop this.");
				return false;
			}
		}
		if (requiredItem != -1
				&& !player.getInventory().containsItemToolBelt(requiredItem, 1)) {
			player.getPackets().sendGameMessage(
					"You need a "
							+ ItemDefinitions.getItemDefinitions(requiredItem)
									.getName().toLowerCase()
	...
	public int processWithDelay(Player player) {
		if (checkAll(player)) {
			...
			} else if (cactus != null || fruit != null || bell != null
					|| bush != null) {
			...
			if (produce != 0) {
				if (patch.yield == 1 && tree == null && fruit == null
						&& bush == null) {
				...
				if (player.hasJujuFarmingBoost()
						&& (herb != null || vineHerb != null)) {
					if (Utilities.getRandom(3) == 2) {
						player.getInventory().addItem(produce, 1);
					}
				}
				if (xp > 0) {
			...
			if (patch.yield <= 0) {
				if (fruit == null && tree == null && bush == null) {
					...
					player.getPackets().sendGameMessage(
							"This " + bush == null ? "tree" : "bush"
									+ " has nothing more to yield.");
				}
				stop(player);
			}
		}
