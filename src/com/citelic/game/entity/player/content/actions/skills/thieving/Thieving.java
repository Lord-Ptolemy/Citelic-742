...
public class Thieving {
		...
		for (int regionId : player.getMapRegionsIds()) {
			...
			for (int npcIndex : npcIndexes) {
				...
				if (!isGuard(npc.getId()) || npc.isUnderCombat()
					...
					continue;
				int distance = Utilities.getDistance(npc.getX(), npc.getY(),
			...
			if (stall.getObjectId() == object.getId()) {
				...
				EngineTaskManager.schedule(new EngineTask() {
					...
					public void run() {
						...
						if (!Engine.getRegion(object.getRegionId())
						...
						if (!gaveItems) {
							player.getInventory().addItem(
									stall.getItem(Utilities
											.getRandom(stall.item.length - 1)),
									Utilities.getRandom(stall.getAmount()));
							player.getSkills().addXp(Skills.THIEVING,
									stall.getExperience());
							gaveItems = true;
							checkGuards(player);
						} else {
							Engine.spawnTemporaryObject(emptyStall,
	...
	public static boolean pickDoor(Player player, GameObject object) {
		...
		int decreasedChance = (Integer) player.getTemporaryAttributtes().get(
				"numbFingers");
		int level = Utilities.getRandom(thievingLevel
				+ (increasedChance - decreasedChance)) + 1;
		double ratio = level / (Utilities.getRandom(45 + 5) + 1);
		...
		Door.handleDoor(player, object, 1500 + Utilities.getRandom(1000));
		return true;
	}
