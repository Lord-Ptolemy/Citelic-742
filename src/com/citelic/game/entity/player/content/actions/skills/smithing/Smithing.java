package com.citelic.game.entity.player.content.actions.skills.smithing;
...
public class Smithing extends Action {
	...
	public static int getHAMMER() {
		return Smithing.HAMMER;
	}

	public static void setHAMMER(int hAMMER) {
		Smithing.HAMMER = hAMMER;
	}
	...
	public boolean process(Player player) {
		if (!player.getInventory()
				.containsItemToolBelt(Smithing.getHAMMER(), 1)) {
	...
	public int processWithDelay(Player player) {
		ticks--;
		player.setNextAnimation(new Animation(83));
	...
	public boolean start(Player player) {
		if ((bar = (ForgingBar) player.getTemporaryAttributtes().get(
				"SmithingBar")) == null)
			return false;
		if (!player.getInventory().containsOneItem(Smithing.getHAMMER(),
				bar.getBarId())) {
			player.getDialogueManager().startDialogue(
					"SimpleMessage",
					"You need a hammer in order to work with a bar of "
	...
	public enum ForgingBar {
		...
		static {
			for (ForgingBar bar : ForgingBar.values()) {
				ForgingBar.bars.put(bar.getBarId(), bar);
			}
		}
	...
	public static class ForgingInterface {
		...
		private static void calculateComponentConfigurations() {
			int base = 18;
			for (int i = 0; i < ForgingInterface.componentChilds.length; i++) {
				if (base == 250) {
					base = 267;
				}
				ForgingInterface.componentChilds[i] = base;
				base += 8;
			}
		}
		...
		private static int getFixedAmount(ForgingBar bar, Item item) {
			...
			int increment = ForgingInterface.getBasedAmount(item);
		...
		public static int getLevels(ForgingBar bar, int slot, Player player) {
			...
			int barAmount = ForgingInterface.getFixedAmount(bar,
					bar.getItems()[slot]);
		...
		public static String[] getStrings(Player player, ForgingBar bar,
			...
			int levelRequired = bar.getLevel()
					+ ForgingInterface.getFixedAmount(bar,
							bar.getItems()[index]);
			int barAmount = ForgingInterface.getActualAmount(levelRequired,
					bar, itemId);
			...
			levelString.append(Utilities.formatPlayerNameForDisplay(name
					.replace(barVariableName + " ", "")));
		...
		public static void handleIComponents(Player player, int componentId) {
			...
			int ticks = -1;
			for (int i = 3; i <= 6; i++) {
				for (int index = 0; index < ForgingInterface.componentChilds.length; index++) {
					if (ForgingInterface.componentChilds[index] + i != componentId) {
						continue;
					}
					slot = index;
					ticks = ForgingInterface.CLICKED_CHILDS[i - 3];
					break;
				}
			}
			if (slot == -1)
		...
		private static void sendComponentConfigs(Player player, ForgingBar bar) {
			for (int i : bar.getComponentChilds()) {
				player.getPackets().sendHideIComponent(
						Smithing.SMITHING_INTERFACE, i - 1, false);
			}
		}

		public static void sendSmithingInterface(Player player, ForgingBar bar) {
			ForgingInterface.calculateComponentConfigurations();
			player.getTemporaryAttributtes().put("SmithingBar", bar);
			ForgingInterface.sendComponentConfigs(player, bar);
			for (int i = 0; i < bar.getItems().length; i++) {
				player.getPackets().sendItemOnIComponent(
						Smithing.SMITHING_INTERFACE,
						ForgingInterface.componentChilds[i],
						bar.getItems()[i].getId(), 1);
				String[] name = ForgingInterface.getStrings(player, bar, i,
						bar.getItems()[i].getId());
				if (name != null) {
					player.getPackets().sendIComponentText(300,
							ForgingInterface.componentChilds[i] + 1, name[0]);
					player.getPackets().sendIComponentText(300,
							ForgingInterface.componentChilds[i] + 2, name[1]);
				}
			}
			player.getPackets().sendIComponentText(
					300,
					14,
					Utilities.formatPlayerNameForDisplay(bar.toString()
							.toLowerCase()) + "");
