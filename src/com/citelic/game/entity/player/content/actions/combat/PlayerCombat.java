...
import com.citelic.game.entity.npc.impl.familiar.Steeltitan;
...
public class PlayerCombat extends Action {
	...
	public static final int isRanging(Player player) {
		...
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart")
					...
					|| name.contains("crystal bow") || name.contains("xil-ul")
					|| name.equalsIgnoreCase("zaryte bow")
					|| name.contains("chinchompa") || name.contains("Bolas") || name.contains("Sling"))
				return 2;
		}
	...
	private void addAttackedByDelay(Entity player) {
		...
		target.setAttackedByDelay(Utilities.currentTimeMillis() + 4000);
	}
	...
	private boolean checkAll(Player player) {
		...
		if (player.getFreezeDelay() >= Utilities.currentTimeMillis()) {
			if (Utilities.colides(player.getX(), player.getY(), size,
		...
		if (player.getPolDelay() >= Utilities.currentTimeMillis()
	...
	private void checkSwiftGlovesEffect(Player player, int hitDelay,
		...
		Engine.sendProjectile(player, target, gfxId, startHeight - 5,
						...
						if (combatXp > 0) {
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								...
								if (weaponId != -1) {
									...
									if (target instanceof Player) {
										if (((Player) target).getPolDelay() >= Utilities
												.currentTimeMillis())
	...
	public void dropAmmo(Player player, int quantity) {
		...
		if (quantity == -2) {
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utilities.getRandom(10) < 2)
						|| (quantity != -3 && Utilities.getRandom(3) > 0)) {
					int capeId = player.getEquipment().getCapeId();
					if (ItemDefinitions.getItemDefinitions(capeId).getName()
							.contains("Ava's")
				...
				if (weaponId == 25202)
					return;
				Engine.updateGroundItem(
						new Item(weaponId, quantity),
						new Tile(target.getCoordFaceX(target.getSize()), target
								.getCoordFaceY(target.getSize()), target.getZ()),
						player);
			}
		} else {
			final int ammoId = player.getEquipment().getAmmoId();
	...
	// public int shieldId = dfsOwner.getEquipment().getShieldId();
	...
	private double getMagicAccuracy(Player e, int baseDamage, int extraDamage) {
		...
		double accuracy = ((magicLevel + (magicBonus * 7))) * prayer;
	...
	// double styleBonus = xpStyle == Skills.STRENGTH ? 3 : xpStyle ==
	// CombatDefinitions.SHARED ? 1 : 0;
	// double otherBonus = 1;
	...
	// otherBonus = 2 - d;
	// }
	// double effectiveStrength = 8 + Math.floor((strengthLvl *
	...
	// player.getPrayer().getRangeMultiplier()) + styleBonus;
	//
	// if (fullVoidEquipped(player, 11664, 11675))
	// effectiveStrenght +=
	// Math.floor((player.getSkills().getLevelForXp(Skills.RANGE) / 5) + 1.6);
	// double strengthBonus =
	// player.getCombatDefinitions().getBonuses()[CombatDefinitions.RANGED_STR_BONUS];
	// double baseDamage = 5 + (((effectiveStrenght + 8) * (strengthBonus + 64))
	// / 64);
	...
	public Entity[] getMultiAttackTargets(Player player, int maxDistance,
		...
		if (target.isAtMultiArea()) {
			y: for (int regionId : target.getMapRegionsIds()) {
				Region region = Engine.getRegion(regionId);
				if (target instanceof Player) {
					...
					for (int playerIndex : playerIndexes) {
						...
						if (p2 == null || p2 == player || p2 == target
							...
							continue;
						/*
						 * if (player.getCurrentFriendChat() != null && clanWars
						 * != null) { if
						 * (clanWars.getFirstPlayers().contains(player) &&
						 * clanWars.getFirstPlayers().contains(p2) ||
						 * clanWars.getSecondPlayers().contains(player) &&
						 * clanWars.getSecondPlayers().contains(p2)) { continue;
						 * } }
						 */
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle,
		...
		int hit = Utilities.random(max_hit);
		if (target instanceof NPC) {
			NPC n = (NPC) target;
		...
		if (usingSpec) {
			...
			if (halfMaxhit > hit && m1 > Math.random() * 2)
				hit = halfMaxhit + Utilities.getRandom(halfMaxhit);
		}
	...
	private int getRangeHitDelay(Player player) {
		return Utilities.getDistance(player.getX(), player.getY(),
				target.getX(), target.getY()) >= 5 ? 2 : 1;
	}
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (spellId == 65535) {
			...
			Engine.sendProjectile(player, target, 2035, 60, 32, 50, 50, 0, 0);
			return 4;
		}
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 98: // Wind Rush
				...
				delayMagicHit(
						2,
						getMagicHit(player,
								getRandomMagicMaxHit(player, baseDamage1)));
				Engine.sendProjectile(player, target, 2699, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 25: // air strike
				player.setNextAnimation(new Animation(14221));
			...
			case 28: // water strike
				...
				delayMagicHit(
						...
						getMagicHit(player,
								getRandomMagicMaxHit(player, baseDamage)));
				Engine.sendProjectile(player, target, 2703, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 36:// bind
				...
				Engine.sendProjectile(player, target, 178, 18, 18, 50, 50, 0, 0);
			...
			case 55:// snare
				...
				if (snareHit.getDamage() > 0
						&& target.getFrozenBlockedDelay() < Utilities
								.currentTimeMillis())
			...
			case 30: // earth strike
				...
				delayMagicHit(
						...
						getMagicHit(player,
								getRandomMagicMaxHit(player, baseDamage)));
				Engine.sendProjectile(player, target, 2718, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 32: // fire strike
				...
				if (target instanceof NPC) {
					...
					if (n.getId() == 9463) // ice verm
						damage *= 2;
					if (n.getId() == 14301) // glacor
						damage *= 2;
				}
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 2729, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 34: // air bolt
			...
			case 39: // water bolt
				...
				delayMagicHit(
						...
						getMagicHit(player,
								getRandomMagicMaxHit(player, baseDamage)));
				Engine.sendProjectile(player, target, 2704, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 42: // earth bolt
				...
				delayMagicHit(
						...
						getMagicHit(player,
								getRandomMagicMaxHit(player, baseDamage)));
				Engine.sendProjectile(player, target, 2719, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 45: // fire bolt
				player.setNextGraphics(new Graphics(2728));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 2731, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 49: // air blast
				player.setNextAnimation(new Animation(14221));
			...
			case 52: // water blast
				...
				Engine.sendProjectile(player, target, 2705, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 58: // earth blast
				...
				Engine.sendProjectile(player, target, 2720, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 63: // fire blast
				player.setNextGraphics(new Graphics(2728));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 2733, 18, 18, 50, 50, 0,
						0);
				return 5;
			...
			case 73: // water wave
				...
				Engine.sendProjectile(player, target, 2706, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 77: // earth wave
				...
				Engine.sendProjectile(player, target, 2721, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 80: // fire wave
				player.setNextGraphics(new Graphics(2728));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 2735, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 86: // teleblock
				if (target instanceof Player
						&& ((Player) target).getTeleBlockDelay() <= Utilities
								.currentTimeMillis()) {
					...
					Hit hit = getMagicHit(player,
							getRandomMagicMaxHit(player, 30));
					delayMagicHit(2, hit);
					Engine.sendProjectile(player, target, 1842, 18, 18, 50, 50,
							0, 0);
				} else {
					player.getPackets().sendGameMessage(
			...
			case 84:// air surge
				...
				Engine.sendProjectile(player, target, 462, 18, 18, 50, 50, 0, 0);
				return 5;
			case 87:// water surge
				player.setNextGraphics(new Graphics(2701));
				...
				Engine.sendProjectile(player, target, 2707, 18, 18, 50, 50, 3,
						0);
				return 5;
			case 89:// earth surge
				...
				Engine.sendProjectile(player, target, 2722, 18, 18, 50, 50, 0,
						0);
				return 5;
			case 91:// fire surge
				player.setNextGraphics(new Graphics(2728));
				...
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 2735, 18, 18, 50, 50, 3,
						0);
				Engine.sendProjectile(player, target, 2736, 18, 18, 50, 50, 20,
						0);
				Engine.sendProjectile(player, target, 2736, 18, 18, 50, 50,
						110, 0);
				return 5;
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				Engine.sendProjectile(player, target, 1019, 18, 18, 50, 30, 0,
						0);
			...
			case 28:// Smoke Rush
				...
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 150)));
				Engine.sendProjectile(player, target, 386, 18, 18, 50, 50, 0, 0);
				return 4;
			case 32:// Shadow Rush
				player.setNextAnimation(new Animation(1978));
				mage_hit_gfx = 379;
				...
				Engine.sendProjectile(player, target, 380, 18, 18, 50, 50, 0, 0);
				return 4;
			case 36: // Miasmic rush
				...
				Engine.sendProjectile(player, target, 1846, 43, 22, 51, 50, 16,
						0);
			...
			case 37: // Miasmic blitz
				...
				Engine.sendProjectile(player, target, 1852, 43, 22, 51, 50, 16,
						0);
				if (target.getTemporaryAttributtes().get("miasmic_immunity") == Boolean.TRUE) {
					return 4;
				}
								...
								public void run() {
									...
									EngineTaskManager.schedule(
											new EngineTask() {
												@Override
												public void run() {
													t.getTemporaryAttributtes()
															.remove("miasmic_immunity");
													stop();
												}
											}, 15);
									stop();
			...
			case 24:// Blood rush
				...
				base_mage_xp = 33;
				blood_spell = true;
				int damage = getRandomMagicMaxHit(player, 170);
				delayMagicHit(2, getMagicHit(player, damage));
				Engine.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
			...
			case 20:// Ice rush
				...
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 180)));
				Engine.sendProjectile(player, target, 362, 18, 18, 50, 50, 0, 0);
				return 4;
				// TODO burst
			case 30:// Smoke burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						delayMagicHit(2, getMagicHit(player, damage));
						Engine.sendProjectile(player, target, 388, 18, 18, 50,
								50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 22:// Ice burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						Engine.sendProjectile(player, target, 366, 43, 0, 120,
								0, 50, 64);
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 25:// Blood Blitz
				...
				base_mage_xp = 45;
				blood_spell = true;
				int blitzDamage = getRandomMagicMaxHit(player, 250);
				delayMagicHit(2, getMagicHit(player, blitzDamage));
				Engine.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
				player.heal(blitzDamage / 4);
				return 4;
			case 21:// Ice Blitz
				player.setNextGraphics(new Graphics(366));
				player.setNextAnimation(new Animation(1978));
				...
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 260)));
				Engine.sendProjectile(player, target, 368, 60, 32, 50, 50, 0, 0);
				return 4;
			case 31:// Smoke barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						delayMagicHit(2, getMagicHit(player, damage));
						Engine.sendProjectile(player, target, 390, 18, 18, 50,
								50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 23: // ice barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						delayMagicHit(
								Utilities.getDistance(player, target) > 3 ? 4
										: 2, hit);
						Engine.sendProjectile(player, target, 368, 60, 32, 50,
								50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
								return false;
	...
	private int meleeAttack(final Player player) {
		...
		/*
		 * if(player.dfsActivated == true && shieldId != 11283) {
		 * player.setdfscoolDown(80); player.dfsActivated = false; }
		 * if(player.dfsActivated == true && shieldId == 11283) {
		 * player.setNextAnimation(new Animation(6696));
		 * player.setNextGraphics(new Graphics(1165)); delayMagicHit(2,
		 * getRegularHit(player, Utilities.random(300)));
		 * World.sendProjectile(player, target, 1166, 46, 31, 50, 60, 0, 0);
		 * player.setdfscoolDown(80); player.dfsActivated = false; return 3; }
		 */
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 23690:
				...
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(player, 50 + Utilities.getRandom(100)),
						getMeleeHit(
								player,
								getRandomMaxHit(player, weaponId, attackStyle,
										false, true, 1.1, true)));
	...
	public int processWithDelay(Player player) {
		...
		if (player.isDfsActivated() && shieldId != 11283) {
			player.setdfscoolDown(80);
			player.setDfsActivated(false);
		}
		if (player.isDfsActivated() && shieldId == 11283) {
			player.setNextAnimation(new Animation(6696));
			player.setNextGraphics(new Graphics(1165));
			delayMagicHit(2, getRegularHit(player, Utilities.random(300)));
			Engine.sendProjectile(player, target, 1166, 46, 31, 50, 60, 0, 0);
			player.setdfscoolDown(80);
			player.setDfsActivated(false);
			return 3;
		}
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 19151:
				...
				Engine.sendProjectile(player, target, 100, 41, 16, 25, 35, 16,
						0);
				delayHit(
						1,
						weaponId,
			...
			case 19148:// guthix bow
				...
				Engine.sendProjectile(player, target, 98, 41, 16, 25, 35, 16, 0);
				delayHit(
						1,
						weaponId,
						attackStyle,
			...
			case 19145:
				...
				Engine.sendProjectile(player, target, 99, 41, 16, 25, 35, 16, 0);
				delayHit(
						1,
						weaponId,
						attackStyle,
			...
			case 10033:
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						Engine.sendProjectile(player, target,
						...
						EngineTaskManager.schedule(new EngineTask() {
							@Override
							public void run() {
								player.setNextGraphics(new Graphics(2739, 0,
										96 << 16));
							}
						}, 2);
						if (!nextTarget) {
			...
			case 18332: // Magic longbow (sighted)
				...
				// player.setNextGraphics(new Graphics(249, 1, 100));
				Engine.sendProjectile(player, target, 249, 41, 16, 31, 35, 16,
						0);
				Engine.sendProjectile(player, target, 249, 41, 16, 25, 35, 21,
						0);
				delayHit(
						2,
						weaponId,
				...
				EngineTaskManager.schedule(new EngineTask() {
					...
					public void run() {
						if ((target.isDead() || player.isDead() || loop > 1)
								&& !Engine.getNPCs().contains(target)) {
							stop();
							return;
						}
						if (loop == 0) {
							...
							Engine.sendProjectile(player, target, 2143, 18, 18,
									50, 50, 0, 0);
							delayHit(
									1,
									weaponId,
			...
			case 15704:
				...
				if (ammoId == 11212) {
					...
					if (damage2 < 80)
						damage2 = 80;
					Engine.sendProjectile(player, target, 1099, 41, 16, 31, 35,
							16, 0);
					Engine.sendProjectile(player, target, 1099, 41, 16, 25, 35,
							21, 0);
					...
					if (damage2 < 50)
						damage2 = 50;
					Engine.sendProjectile(player, target, 1101, 41, 16, 31, 35,
							16, 0);
					Engine.sendProjectile(player, target, 1101, 41, 16, 25, 35,
							21, 0);
					delayHit(2, weaponId, attackStyle,
							getRangeHit(player, damage));
					delayHit(3, weaponId, attackStyle,
			...
			case 14684: // zanik cbow
				...
				Engine.sendProjectile(player, target, 2001, 41, 41, 41, 35, 0,
						0);
				delayHit(
						2,
						weaponId,
			...
			case 13882:
				...
				Engine.sendProjectile(player, target, 1837, 41, 41, 41, 35, 0,
						0);
			...
			case 13957:// morigan thrown axe
				...
				Engine.sendProjectile(player, target, 1839, 41, 41, 41, 35, 0,
						0);
				delayHit(
						2,
						weaponId,
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("deathtouched dart")) {
					if (target instanceof Player || player.isAdmin()) {
						player.stopAll();
						player.faceEntity(target);
						player.getPackets().sendGameMessage(
								"You can't throw " + weaponName
										+ " on Citelic players.");
						return 0;
					}
				}
				if (weaponName.contains("throwing axe")
					...
					if (weaponName.contains("deathtouched dart")) {
						if (target instanceof Player) {
							...
							player.getPackets()
									.sendGameMessage(
											"You can't throw deathtouched darts at players.");
						} else {
							NPC n = (NPC) target;
							n.setCapDamage(30000);
							n.setNextGraphics(new Graphics(3428));
							getTarget().addReceivedDamage(player,
									n.getHitpoints());
					...
					Engine.sendProjectile(player, target, 27, 38, 36, 41, 32,
							5, 0);
					delayHit(2, weaponId, attackStyle,
							getRangeHit(player, damage));
					if (weaponId != 4740)
					...
					if (Utilities.getRandom(player.getSkills().getLevel(
						...
						player.setNextGraphics(new Graphics(2138));
						Engine.sendProjectile(player, target, 2143, 18, 18, 60,
								30, 0, 0);
						delayHit(
								1,
								weaponId,
					...
					Engine.sendProjectile(player, target, 249, 41, 36, 41, 35,
							0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle,
							true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					...
					Engine.sendProjectile(player, target, 468, 41, 41, 41, 35,
							0, 0);
					int delay = 15000;
					if (target instanceof Player) {
						Player p = (Player) target;
