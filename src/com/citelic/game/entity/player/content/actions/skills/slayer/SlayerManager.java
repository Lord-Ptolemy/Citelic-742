...
import com.citelic.game.entity.player.content.actions.skills.slayer.Slayer.SlayerTask;
import com.citelic.utility.Utilities;

/**
 * @author Ridiculous Slayer Manager
 */
public class SlayerManager implements Serializable {
	...
	protected int completedTasks, maximumTaskCount, currentTaskCount;
	...
	private void addRemovedTask(int slot) {
		...
		sendSlayerInterface(SlayerManager.ASSIGNMENT_INTERFACE);
	...
	public Object[] calculateTask() {
		...
		if (currentMaster == SlayerMaster.KURADAL) {
			if (player.isCompletedFightCaves()
					|| learnedAbilities[SlayerManager.ICE_STYKE]) {
				tasks.add(SlayerTask.ICE_STRYKEWYRM);
			}
			if (learnedAbilities[SlayerManager.AQUANTITIES]) {
				tasks.add(SlayerTask.AQUANITE);
			}
		}
		while (true) {
			SlayerTask task = tasks.get(Utilities.random(tasks.size()));
			if (socialPlayer != null) {
				if (player.getSkills().getLevel(Skills.SLAYER) >= task
						.getLevelRequried()
						&& socialPlayer.getSkills().getLevel(Skills.SLAYER) >= task
								.getLevelRequried())
					return new Object[] {
							task,
							Utilities.random(currentMaster.getTasksRange()[0],
	...
	public void createSocialGroup(boolean initial) {
		...
		if (initial) {
			...
			player.getPackets().sendGameMessage(
					socialPlayer.getDisplayName()
							+ " to join your Slayer group.");
		} else {
			player.getPackets().sendGameMessage(
	...
	public void setCurrentTask(boolean initial) {
		...
		this.setCurrentTask((SlayerTask) futureTask[0], (int) futureTask[1]);
		checkKillsLeft();
		if (initial) {
			if (socialPlayer != null) {
	...
	public void handleRewardButtons(int interfaceId, int componentId) {
		if (componentId == 15) {
			sendSlayerInterface(SlayerManager.BUY_INTERFACE);
		} else if (componentId == 16) {
			sendSlayerInterface(SlayerManager.ABILITIES_INTERFACE);
		} else if (componentId == 17) {
			sendSlayerInterface(SlayerManager.ASSIGNMENT_INTERFACE);
		} else if (componentId == 14) {
			if (interfaceId == SlayerManager.ASSIGNMENT_INTERFACE) {
				sendSlayerInterface(SlayerManager.ABILITIES_INTERFACE);
			} else {
				sendSlayerInterface(SlayerManager.ASSIGNMENT_INTERFACE);
			}
		}
		if (interfaceId == SlayerManager.BUY_INTERFACE) {
		...
		} else if (interfaceId == SlayerManager.ABILITIES_INTERFACE) {
			if (componentId >= 73 && componentId <= 78) {
				unlockAbility(componentId - 73);
			}
		} else if (interfaceId == SlayerManager.ASSIGNMENT_INTERFACE) {
	...
	public boolean hasLearnedBroad() {
		return learnedAbilities[SlayerManager.BROAD_TIPS];
	}

	public boolean hasLearnedQuickBlows() {
		return learnedAbilities[SlayerManager.QUICK_BLOWS];
	}

	public boolean hasLearnedRing() {
		return learnedAbilities[SlayerManager.RING_OF_SLAYING];
	}

	public boolean hasLearnedSlayerHelmet() {
		return learnedAbilities[SlayerManager.SLAYER_HELMET];
	}
	...
	public void invitePlayer(Player otherPlayer) {
		...
		player.getTemporaryAttributtes().put("social_request", otherPlayer);
		player.getPackets().sendGameMessage(
				"You have invited " + otherPlayer.getDisplayName()
						+ " to join your Slayer group.");
		otherPlayer.getPackets().sendMessage(
				117,
				"You have received an invitation to join "
						+ player.getDisplayName() + "'s Slayer group.", player);
	}
	...
	private void sendPoints(int interfaceId) {
		if (interfaceId == SlayerManager.ASSIGNMENT_INTERFACE) {
			player.getPackets().sendIComponentText(interfaceId, 19,
					"" + slayerPoints);
		} else if (interfaceId == SlayerManager.BUY_INTERFACE) {
	...
	public void sendSlayerInterface(int interfaceId) {
		...
		if (interfaceId == SlayerManager.ASSIGNMENT_INTERFACE) {
			player.getPackets().sendHideIComponent(
					SlayerManager.ASSIGNMENT_INTERFACE, 42, true);
			for (int index = 0; index < canceledTasks.length; index++) {
				SlayerTask task = canceledTasks[index];
				if (task != null) {
					canceledTasksCount++;
					player.getPackets().sendIComponentText(
							SlayerManager.ASSIGNMENT_INTERFACE, 31 + index,
							task.getName());
				} else {
					player.getPackets().sendHideIComponent(
							SlayerManager.ASSIGNMENT_INTERFACE, 37 + index,
							true);
				}
			}
		} else if (interfaceId == SlayerManager.ABILITIES_INTERFACE) {
			for (int index = 0; index < learnedAbilities.length; index++) {
				...
				if (hasUnlocked) {
					Object[] data = SlayerManager.ABILITY[index];
					player.getPackets().sendIComponentText(
							SlayerManager.ABILITIES_INTERFACE, (int) data[0],
							"You have already unlocked this ability.");
					player.getPackets().sendIComponentText(
							SlayerManager.ABILITIES_INTERFACE, (int) data[1],
							"");
					player.getPackets()
							.sendHideIComponent(
									SlayerManager.ABILITIES_INTERFACE,
									73 + index, true);
				}
			}
		}
	...
	public void skipCurrentTask() {
		player.print("Your slayer task has been reset.");
	...
	private void unlockAbility(int slot) {
		if (learnedAbilities[slot])
			return;
		else if (!removePoints((int) SlayerManager.ABILITY[slot][2]))
			return;
		learnedAbilities[slot] = true;
		sendSlayerInterface(SlayerManager.ABILITIES_INTERFACE);
		player.getPackets().sendGameMessage(
				"You have unlocked the ability "
						+ SlayerManager.ABILITY[slot][3] + "!");
	}
}