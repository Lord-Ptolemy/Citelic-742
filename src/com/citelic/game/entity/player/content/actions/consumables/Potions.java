package com.citelic.game.entity.player.content.actions.consumables;
...
public final class Potions {
	...
	public static enum Pot {
		...
		ANTI_POISION(new int[] { 2446, 175, 177, 179 }, Effects.ANTIPOISON), SUPER_ANTI_POISION(
				new int[] { 2448, 181, 183, 185 }, Effects.SUPER_ANTIPOISON),
		...
		RECOVER_SPECIAL() {
			...
			public boolean canDrink(Player player) {
				...
				Long time = (Long) player.getTemporaryAttributtes().get(
						"Recover_Special_Pot");
				if (time != null
						&& Utilities.currentTimeMillis() - time < 30000) {
					player.getPackets().sendGameMessage(
	...
	public static boolean mixPot(Player player, Item fromItem, Item toItem,
			int fromSlot, int toSlot) {
		if (fromItem.getId() == VIAL || toItem.getId() == VIAL) {
			Pot pot = getPot(fromItem.getId() == VIAL ? toItem.getId()
					: fromItem.getId());
			...
			if (doses == 1) {
				...
				player.getPackets().sendGameMessage(
						"You pour from one container into the other.", true);
				return true;
			}
			...
			player.getPackets().sendGameMessage(
					"You split the potion between the two vials.", true);
			return true;
		}
			...
			player.getPackets()
					.sendGameMessage(
							dosesLeft == 0 ? "You have finished your "
									+ (pot.isFlask() ? "flask and the glass shatters to pieces."
											: pot.isPotion() ? "potion." : item
													.getName().toLowerCase()
													+ ".")
									: "You have " + dosesLeft
