...
public class Smelting extends Action {
	...
	public enum SmeltingBar {
		...
		static {
			for (SmeltingBar bar : SmeltingBar.values()) {
				...
				System.out.println("Bars: " + bar.getItemsRequired()
						+ ", buttonId: " + bar.getButtonId());
			}
		}
	...
	public boolean isSuccessFull(Player player) {
		if (bar == SmeltingBar.IRON) {
			if (player.getEquipment().getItem(Equipment.SLOT_RING) != null
				...
				return Utilities.getRandom(100) <= (player.getSkills()
						.getLevel(Skills.SMITHING) >= 45 ? 80 : 50);
			}
		}
		return true;
	}
	...
	public boolean process(Player player) {
		...
		if (player.getInterfaceManager().containsScreenInter()
			...
			player.getPackets()
					.sendGameMessage(
							"Please finish what you're doing before doing this action.");
			return false;
		}
