...
public class PickPocketAction extends Action {
	...
	private boolean checkAll(Player player) {
		...
		if (npc.getAttackedBy() != null
				&& npc.getAttackedByDelay() > Utilities.currentTimeMillis()) {
	...
	private Animation getAnimation() {
		switch (index) {
		case 0:
			return PickPocketAction.PICKPOCKETING_ANIMATION;
		case 1:
			return PickPocketAction.DOUBLE_LOOT_ANIMATION;
		case 2:
			return PickPocketAction.TRIPLE_LOOT_ANIMATION;
		case 3:
			return PickPocketAction.QUADRUPLE_LOOT_ANIMATION;
		}
		return null;
	}
	...
	private Graphics getGraphics() {
		...
		case 0:
			return null;
		case 1:
			return PickPocketAction.DOUBLE_LOOT_GFX;
		case 2:
			return PickPocketAction.TRIPLE_LOOT_GFX;
		case 3:
			return PickPocketAction.QUADRUPLE_LOOT_GFX;
		}
		return null;
	}
	...
	private boolean isSuccesfull(Player player) {
		...
		int level = Utilities.getRandom(thievingLevel + increasedChance);
		double ratio = level
				/ (Utilities.random(npcData.getThievingLevels()[0] + 6) + 1); // dammit
		// u
		// fail
		...
		if (!isSuccesfull(player)) {
			player.getPackets().sendGameMessage(
					"You fail to pick the "
							...
							+ "'s pocket.");
			npc.setNextAnimation(PickPocketAction.STUN_ANIMATION);
			...
			if (Utilities.getRandom(115) < 1) {
				player.getInventory().addItem(10075, 1);
			}
			if (Utilities.getRandom(125) < 1) {
				player.getInventory().addItem(15349, 1);
			}
			...
			for (int i = 0; i <= index; i++) {
				Item item = npcData.getLoot()[Utilities.random(npcData
						.getLoot().length)];
