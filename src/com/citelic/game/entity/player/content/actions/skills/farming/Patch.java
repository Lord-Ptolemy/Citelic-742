...
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.AllotmentPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.BellPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.BushPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.CactusPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.FlowerPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.FruitTreePatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.HerbPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.HopPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.MushroomPatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.TreePatch;
import com.citelic.game.entity.player.content.actions.skills.farming.PatchConstants.VineHerbPatch;
...
public class Patch implements Serializable {
	...
	public void grow() {
		...
		if (FruitTreePatch.forId(currentSeed) != null
				|| TreePatch.forId(currentSeed) != null) {
		...
		if (state < maxState - 1 && !dead && !diseased
				&& protectedPatch == false) {
			...
			if (Utilities.getRandomDouble(100.0) < diseaseChance) {
				diseased = true;
			}
		}
		...
		if (TreePatch.forId(currentSeed) != null) {
			if (player.getInventory().containsItemToolBelt(
				...
				if (grown) {
					...
					player.getPackets()
							.sendGameMessage(
									"The tree needs to have grown before you can clear it.");
				}
			} else {
				player.getPackets().sendGameMessage(
						"You need a spade to clear the patch.");
			}
		}
		return;
	}
	...
	public void handleCompost(Player player, int itemId) {
		if (composted == 0) {
			...
			player.getPackets().sendGameMessage(
					"You pour some compost on the patch.");
			composted = itemId == 6032 ? 1 : 2;
		} else {
			player.getPackets().sendGameMessage(
					"This patch is already composted.");
		}
	}
	...
	public void handleCuring(Player player, int itemId) {
		if (diseased) {
			...
			player.getPackets().sendGameMessage(
					"This plant doesn't need curing.");
		}
		updatePatch(player);
	}
	...
	public void handleInspection(Player player) {
		if (!raked) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"This patch needs weeding.");
			return;
		}
		if (dead) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"This patch is dead. Dig it up and plant a new one.");
			return;
		}
		if (diseased) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"This patch is diseased. It needs curing.");
			return;
		}
		if (configIndex == 0) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"The patch is cleared and ready for planting.");
			return;
		}
		if (!grown) {
			switch (state) {
			case 0:
				player.getDialogueManager().startDialogue(
						...
						"Some "
								+ ItemDefinitions
										.getItemDefinitions(currentSeed)
										.getName().toLowerCase()
								+ " have just been planted here.");
				break;
			default:
				player.getDialogueManager().startDialogue(
						"SimpleMessage",
						"Some "
								+ ItemDefinitions
										.getItemDefinitions(currentSeed)
										.getName().replace(" seed", "")
										.toLowerCase() + " are growing here.");
			}
			updatePatch(player);
			return;
		}
		player.getDialogueManager().startDialogue("SimpleMessage",
				"This patch looks ready to harvest.");
	}

	public void handleOption1(Player player) {
		if (!raked) {
			if (!player.getInventory().containsItemToolBelt(
					PatchConstants.RAKE, 1)) {
				player.getPackets().sendGameMessage(
						"You need a rake to rake the patch.");
				return;
			}
			...
			if (FruitTreePatch.forId(currentSeed) != null) {
				if (yield == 0) {
					...
					if (axeAnim != -1) {
						...
						if (player.hasScrollOfLife) {
							if (Utilities.getRandom(10) == 5) {
								player.getPackets().sendGameMessage(
										"You recovered a seed from the patch!");
						...
						player.getPackets()
								.sendGameMessage(
										"You need a hatchet that you are able to use with your woodcutting level to cut this tree.");
					}
					return;
				}
				if (!healthChecked) {
					...
					player.getSkills().addXp(Skills.FARMING,
							FruitTreePatch.forId(currentSeed).getXp());
					healthChecked = true;
				} else {
					player.getActionManager().setAction(
							new HarvestAction(player, this));
				}
				...
				if (yield == 0) {
					if (player.getInventory().containsItemToolBelt(
						...
						player.getPackets().sendGameMessage(
								"You need a spade to clear the stump.");
					}
					return;
				}
				if (!healthChecked) {
					player.getPackets()
							.sendGameMessage(
									"You examine the tree for signs of disease and find that it is in perfect health.");
					player.getSkills().addXp(Skills.FARMING,
							TreePatch.forId(currentSeed).getXp());
					healthChecked = true;
				} else {
					player.getActionManager().setAction(
							new HarvestAction(player, this));
				...
				if (BellPatch.forId(currentSeed) != null) {
					if (player.getEquipment().wearingGloves()) {
						...
						player.applyHit(new Hit(player, 20,
	...
	public void handlePlanting(Player player, int seedId) {
		...
		if (!raked) {
			player.getPackets().sendGameMessage(
					"This patch needs weeding first.");
			return;
		}
		if (currentSeed != -1) {
			player.getPackets().sendGameMessage(
					"There is already something growing in this patch.");
			return;
		}
		if (objectId == 18816) {
			protectedPatch = true;
		...
		if (objectId == 7572) {
			...
			if (bell != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < bell
						.getRequirement()) {
					player.getPackets()
							.sendGameMessage(
				...
				currentSeed = bell.getSeed();
				configIndex = bell.getConfigId();
				maxState = 4;
				yield = 1;
				player.getSkills().addXp(Skills.FARMING, bell.getXp() / 4);
				player.getPackets().sendGameMessage(
						"You plant the seeds in the patch.");
			} else {
				player.getPackets().sendGameMessage(
			...
			if (vineHerb != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < vineHerb
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ vineHerb.getRequirement()
									+ " to plant that.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(vineHerb.getSeed(), 1);
				currentSeed = vineHerb.getSeed();
				configIndex = vineHerb.getConfigId();
				maxState = 4;
		...
		if (objectId >= 7577 && objectId <= 7580) {
			...
			if (bush != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < bush
						.getRequirement()) {
					player.getPackets()
							.sendGameMessage(
									"You need a farming level of "
				...
				currentSeed = bush.getSeed();
				configIndex = bush.getConfigId();
				maxState = bush.getMaxState();
				yield = 4;
				if (seedId == 5106) {
					protectedPatch = true;
				}
		...
		if (objectId >= 7847 && objectId <= 7850) {
			...
			if (flower != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < flower
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ flower.getRequirement()
									+ " to plant that.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(flower.getSeed(), 1);
				currentSeed = flower.getSeed();
				configIndex = flower.getConfigId();
				maxState = 4;
		...
		if (objectId == 18816 || objectId >= 8150 && objectId <= 8153) {
			...
			if (herb != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < herb
						.getRequirement()) {
					player.getPackets()
							.sendGameMessage(
									"You need a farming level of "
				...
				currentSeed = herb.getSeed();
				configIndex = herb.getConfigId();
				maxState = 4;
		...
		if (objectId >= 8550 && objectId <= 8557) {
			...
			if (allotment != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < allotment
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ allotment.getRequirement()
									+ " to plant that.");
					return;
				}
				if (!player.getInventory().containsItem(allotment.getSeed(), 3)) {
					player.getPackets().sendGameMessage(
							"You need 3 seeds to plant an allotment.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(allotment.getSeed(), 3);
				currentSeed = allotment.getSeed();
				configIndex = allotment.getConfigId();
				maxState = allotment.getMaxStage();
		...
		if (objectId == 8337) {
			...
			if (mushroom != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < mushroom
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ mushroom.getRequirement()
									+ " to plant that.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(mushroom.getSeed(), 1);
				currentSeed = mushroom.getSeed();
				configIndex = mushroom.getConfigId();
				maxState = 6;
		...
		if (objectId == 7771) {
			...
			if (cactus != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < cactus
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ cactus.getRequirement()
									+ " to plant that.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(cactus.getSeed(), 1);
				currentSeed = cactus.getSeed();
				configIndex = cactus.getConfigId();
				maxState = 8;
		...
		if (objectId == 7964 || objectId == 7965 || objectId == 7962
				|| objectId == 7963 || objectId == 28919 || objectId == 56667) {
			FruitTreePatch tree = FruitTreePatch.forId(seedId);
			if (tree != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < tree
						.getRequirement()) {
					player.getPackets()
							.sendGameMessage(
									"You need a farming level of "
				...
				currentSeed = tree.getSeed();
				configIndex = tree.getConfigId();
				maxState = 6;
				yield = 6;
		...
		if (objectId == 8389 || objectId == 19147 || objectId == 8391
				|| objectId == 8388 || objectId == 8390) {
			...
			if (tree != null) {
				...
				currentSeed = tree.getSeed();
				configIndex = tree.getConfigId();
				maxState = tree.getMaxState();
				yield = Utilities.random(3, 15);
		...
		if (objectId >= 8173 && objectId <= 8176) {
			...
			if (hop != null) {
				if (player.getSkills().getLevel(Skills.FARMING) < hop
						.getRequirement()) {
					player.getPackets().sendGameMessage(
							"You need a farming level of "
									+ hop.getRequirement() + " to plant that.");
					return;
				}
				if (!player.getInventory().containsItem(hop.getSeed(), 4)) {
					player.getPackets().sendGameMessage(
							"You need 4 seeds to plant a hop.");
					return;
				}
				player.setNextAnimation(new Animation(2291));
				player.getInventory().deleteItem(hop.getSeed(), 4);
				currentSeed = hop.getSeed();
				configIndex = hop.getConfigId();
				maxState = hop.getMaxStage();
	...
	public void handleWatering(Player player, int itemId) {
		if (itemId == 5331) {
			player.getPackets().sendGameMessage(
					"You need to fill your watering can.");
			return;
		}
		...
		if (AllotmentPatch.forId(currentSeed) != null
				|| FlowerPatch.forId(currentSeed) != null
				|| HopPatch.forId(currentSeed) != null) {
			if (state < maxState) {
				...
				player.getPackets().sendGameMessage(
						"This patch doesn't look like it needs water.");
			}
		} else {
	...
	public void updatePatch(Player player) {
		...
		if (!raked || configIndex == 0) {
			...
			if (BushPatch.forId(currentSeed) != null) {
				...
				if (healthChecked) {
					finalConfig = FruitTreePatch.forId(currentSeed)
							.getPickFruit() - 6 + yield;
				} else {
					if (state != maxState) {
						finalConfig = configIndex + state;
					} else {
