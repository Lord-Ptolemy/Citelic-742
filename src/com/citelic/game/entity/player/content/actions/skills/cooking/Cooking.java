package com.citelic.game.entity.player.content.actions.skills.cooking;
...
public class Cooking extends Action {
	...
	private int FIRE_COOKING = 897, RANGE_COOKING = 883;
	...
	private boolean isBurned(Cookables cook, Player player) {
		...
		return Utilities.random(34) <= levelsToStopBurn;
	}
	...
	public int processWithDelay(Player player) {
		amount--;
		player.setNextAnimation(new Animation(object.getDefinitions().name
				.equals("Fire") ? FIRE_COOKING : RANGE_COOKING));
		if (player.getSkills().getLevel(Skills.COOKING) >= cook.getBurningLvl() ? false
	...
	public enum Cookables {
		...
		static {
			for (Cookables ingredient : Cookables.values()) {
				Cookables.ingredients.put((short) ingredient.getRawItem()
						.getId(), ingredient);
			}
		}
		...
		public static Cookables forId(short itemId) {
			return Cookables.ingredients.get(itemId);
		}
