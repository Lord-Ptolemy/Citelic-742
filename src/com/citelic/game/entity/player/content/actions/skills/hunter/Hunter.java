package com.citelic.game.entity.player.content.actions.skills.hunter;
...
public class Hunter {
	...
	public enum FlyingEntities {
		...
		SPIRIT_IMPLING(7866, 15513, 227, 321, 54, new Item[] {
			...
			public void effect(Player player) {
				if (Utilities.getRandom(1) == 0) {
					Item charm = CHARMS[Utilities.random(CHARMS.length)];
					int charmAmount = Utilities.getRandom(charm.getAmount());
					...
					player.getInventory().addItem(
							new Item(charm.getId(), charmAmount));
				}
			}
		},
		...
		KINGLY_IMPLING(7903, 15517, 434, 617, 91, new Item[] {
				new Item(1705, Utilities.random(3, 11)), new Item(1684, 3),
				new Item(1618, Utilities.random(17, 34)), new Item(990, 2) },
				new Item[] { new Item(1631, 1), new Item(1615, 1),
						new Item(9341, 40 + Utilities.getRandom(30)),
						new Item(9342, 57), new Item(15511, 1),
						new Item(15509, 1), new Item(15505, 1),
						new Item(15507, 1), new Item(15503, 1),
						new Item(11212, 40 + Utilities.random(104)),
						new Item(9193, 62 + Utilities.random(8)),
						new Item(11230, Utilities.random(182, 319)),
						new Item(11232, 70),
						new Item(1306, Utilities.random(1, 2)),
						new Item(1249, 1) }, null,
						...
						EngineTaskManager.schedule(new EngineTask() {
							...
							public void run() {
								...
								for (int trycount = 0; trycount < 10; trycount++) {
									teleTile = new Tile(npc, 3);
									if (Engine.isTileFree(npc.getZ(),
	...
	public static void openJar(Player player, FlyingEntities instance, int slot) {
		boolean isImpling = instance.toString().toLowerCase()
				.contains("impling");
		if (isImpling) {
			int chance = Utilities.getRandom(100);
			Item item = null;
			Item[] list = null;
			if (chance <= 60) {
				...
				if (list != null)
					item = list[Utilities
							.random(instance.getRarleyCommon().length)];
			} else if (chance < 80) {
				list = instance.getCommon();
				if (list != null)
					item = list[Utilities.random(instance.getCommon().length)];
			} else if (chance < 100) {
				list = instance.getRare();
				if (list != null)
					item = list[Utilities.random(instance.getRare().length)];
			} else {
				list = instance.getRarleyCommon();
				if (item == null)
					item = list[Utilities
			...
			if (item != null)
				player.getInventory().addItemDrop(item.getId(),
						Utilities.random(item.getAmount()) + 1);
		...
		if (Utilities.getRandom(4) == 0) {
			...
			player.applyHit(new Hit(player, 10, Hit.HitLook.REGULAR_DAMAGE));
		}
	}
	...
	public static void createLoggedObject(Player player, GameObject object,
			boolean kebbits) {
		...
		// everything :L
		if (Engine.removeTemporaryObject(object, 300000, false)) {// five minute
																	// delay
			Engine.spawnObjectTemporary(new GameObject(kebbits ? 19206 : -1,
	...
	public static boolean isSuccessful(Player player, int dataLevel,
		 ...
		 * Utilities.random(hunterlevel + increasedProbability) + 1;
		 * 
		 * int chance = level * 100 / (dataLevel * 2);
		 * 
		 * if (Utilities.random(100) > chance) return false;
		 */

		return Utilities.random(4) != 0;
	}
}