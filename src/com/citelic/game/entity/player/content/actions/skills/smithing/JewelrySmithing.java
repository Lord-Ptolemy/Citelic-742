...
public class JewelrySmithing {
	...
	private static final int[][] ITEMS = {
			{ 1635, 1637, 1639, 1641, 1643, 1645, 1647 },
			{ 1654, 1656, 1658, 1660, 1662, 1664, 1666 },
			{ 1673, 1675, 1677, 1679, 1681, 1685 },
			{ 11069, 11072, 11076, 11085, 11092, 11115, 11067 } };
					...
					player.getActionManager().setAction(new Action() {
						...
						public boolean process(Player player) {
							...
							if (player.getSkills().getLevel(Skills.CRAFTING) < level) {
								...
								player.getPackets()
										.sendGameMessage(
												"You need a gold bar in order to create jewellery.");
								return false;
							} else if (actionSecondaryIndex != 0
									&& !player.getInventory().containsItem(
											GEMS[actionSecondaryIndex], 1)) {
								player.getPackets()
										.sendGameMessage(
	...
	public static void ringTransformation(Player player, final int itemId) {
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				player.stopAll();
				player.lock(2);
				int transformationId = EGG_IDS[Utilities.random(EGG_IDS.length)];
				if (itemId == 6583)
