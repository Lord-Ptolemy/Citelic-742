...
public class PotteryWheel extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentIndex > PotteryWheel.PRODUCTS.length) {
			end();
			return;
		}
		player.getActionManager().setAction(new Action() {
			...
			public int processWithDelay(Player player) {
				...
				player.getSkills().addXp(Skills.CRAFTING,
						PotteryWheel.EXPERIENCE[componentIndex]);
				player.getInventory().deleteItem(PotteryWheel.SOFT_CLAY, 1);
				player.getInventory().addItem(
						PotteryWheel.PRODUCTS[componentIndex], 1);
				return 4;
			}
			...
			public boolean start(Player player) {
				...
				int clayAmount = player.getInventory().getNumberOf(
						PotteryWheel.SOFT_CLAY);
				if (clayAmount == 0) {
					PotteryWheel.this.end();
					player.getPackets().sendGameMessage(
							"You need soft clay in order to spin any pot.");
					return false;
				}
				final int levelReq = PotteryWheel.LEVELS[componentIndex];
	...
	public void start() {
		SkillsDialogue
				.sendSkillsDialogue(
						...
						28, PotteryWheel.PRODUCTS, new ItemNameFilter() {
							int count = 0;

							@Override
							public String rename(String name) {
								int levelRequired = PotteryWheel.LEVELS[count++];
								if (PotteryWheel.this.player.getSkills()
