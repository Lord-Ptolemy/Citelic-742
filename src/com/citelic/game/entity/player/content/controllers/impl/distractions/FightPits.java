...
import com.citelic.game.entity.npc.impl.fightpits.FightPitsNPC;
import com.citelic.game.entity.npc.impl.fightpits.TzKekPits;
...
public final class FightPits {
	...
	private static Tile[] GAME_TELEPORTS = { new Tile(4577, 5086, 0),
			new Tile(4571, 5083, 0), new Tile(4564, 5086, 0),
			new Tile(4564, 5097, 0), new Tile(4571, 5101, 0),
	...
	public static void enterArena(Player player) {
		...
		player.setNextTile(new Tile(GAME_TELEPORTS[Utilities
			...
			if (type == 0) {
				...
				if (player.hasSkull() && player.getSkullId() == 1) {// if has
					...
					if (!player.getInventory().addItem(6529, tokkul)
							&& type == 1)
						Engine.addGroundItem(new Item(6529, tokkul), new Tile(
								4585, 5076, 0), player, true, 180, true);
				}
				if (type == 1) {
					player.lock(5);
					player.addWalkSteps(4585, 5076, 5, false);
				} else if (type == 2)
					player.setNextTile(new Tile(new Tile(4592, 5073, 0), 2));
			}
			refreshFoes();
			checkPlayersAmmount();
