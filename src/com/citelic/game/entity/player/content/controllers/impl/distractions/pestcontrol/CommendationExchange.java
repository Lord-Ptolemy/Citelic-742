...
public class CommendationExchange {// 1875 TODO
	...
	private static void addXPForSkill(Player player, int skill, int rate) {
		...
		player.getDialogueManager().startDialogue(
				"SimpleMessage",
				"You gain " + Utilities.getFormattedNumber((int) experience)
	...
	public static void handleButtonOptions(Player player, int componentId) {
		if (componentId == 68) {
			...
			player.getInventory().addItem(
					Herbs.values()[Utilities.random(5)].getHerbId() + 1,
					Utilities.random(4));
			player.getInventory().addItem(
			...
			if (player.getSkills().getLevelForXp(Skills.MINING) < 25) {
				...
				return;
			player.getInventory().addItem(441, Utilities.random(20));
			player.getInventory().addItem(454, Utilities.random(30));
			player.getPackets().sendGameMessage(
