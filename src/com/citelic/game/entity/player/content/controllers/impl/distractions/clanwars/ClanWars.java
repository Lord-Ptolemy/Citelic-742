...
public final class ClanWars implements Serializable {
			...
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					MapBuilder.copyAllPlanesMap(areaType.getSouthWestTile()
							...
							height);
					baseLocation = new Tile(newCoords[0] << 3,
							newCoords[1] << 3, 0);
	...
	public void leave(Player p, boolean ingame) {
		...
		p.setNextTile(new Tile(2992, 9676, 0));
	...
	public void endWar() {
		...
		for (Player player : firstPlayers) {
			...
			player.getInterfaceManager().removeInterface(resized ? 746 : 548,
					resized ? 1 : 11);
