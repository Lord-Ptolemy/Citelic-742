...
public class DuelArena extends Controller {
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		...
		player.getPackets().sendIComponentText(ifFriendly ? 637 : 631, ifFriendly ? 16 : 38, " " + Utilities.formatPlayerNameForDisplay(target.getUsername()));
	...
	private void startEndingTeleport(Player player) {
		Tile tile = LOBBY_TELEPORTS[Utilities.random(LOBBY_TELEPORTS.length)];
		Tile teleTile = tile;
		for (int trycount = 0; trycount < 10; trycount++) {
			teleTile = new Tile(tile, 2);
			if (Engine.canMoveNPC(tile.getPlane(), teleTile.getX(),
	...
	private void beginBattle(boolean started) {
		if (started) {
			...
			int random = Utilities.getRandom(1);
	...
	private Tile[] getPossibleTiles() {
		final int arenaChoice = Utilities.getRandom(2);
		...
		int finalX = arenaBoundariesX[arenaChoice]
				+ Utilities.getRandom(maxOffsetX[arenaChoice]);
		int finalY = arenaBoundariesY[arenaChoice]
				+ Utilities.getRandom(maxOffsetY[arenaChoice]);
		locations[0] = (new Tile(finalX, finalY, 0));
		if (player.getLastDuelRules().getRule(25)) {
			int direction = Utilities.getRandom(1);
			if (direction == 0) {
				finalX--;
			} else {
				finalY++;
