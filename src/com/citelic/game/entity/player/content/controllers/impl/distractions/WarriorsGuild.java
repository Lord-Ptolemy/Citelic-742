...
import com.citelic.game.entity.npc.impl.others.AnimatedArmor;
...
public class WarriorsGuild extends Controller {
	...
	public static class WarriorTimer extends TimerTask {
		...
		public void run() {
			...
			if (ticks % 14 == 0) {
				...
				Engine.sendObjectAnimation(CATAPULT, new Animation(4164));
				projectileType = (byte) Utilities.random(4);
				Engine.sendProjectile(CATAPULT_PROJECTILE_BASE,
						CATAPULT_TARGET, 679 + projectileType, 85, 0, 15, 16,
						15, 0);
			}
		}
		...
		private void switchDummieAction() {
			int index = Utilities.random(DUMMY_LOCATIONS.length);
			Engine.spawnObjectTemporary(
					new GameObject(Utilities.random(15624, 15630), 10,
	 ...
	 */
	private static final Tile[] DUMMY_LOCATIONS = { new Tile(2860, 3549, 0),
			new Tile(2860, 3547, 0), new Tile(2859, 3545, 0),
			new Tile(2857, 3545, 0), new Tile(2855, 3546, 0),
			new Tile(2855, 3548, 0), new Tile(2856, 3550, 0),
			new Tile(2858, 3550, 0) };
	private static final int[] DUMMY_ROTATIONS = { 1, 1, 2, 2, 3, 3, 0, 0 };

	/**
	 * Catapult Activity (Defence)
	 */
	private static final Tile CATAPULT_TARGET = new Tile(2842, 3541, 1);
	private static final NPC CATAPULT_PROJECTILE_BASE = new NPC(1957, new Tile(
			2842, 3550, 1), -1, false);
	private static final Animation[] DEFENCIVE_ANIMATIONS = new Animation[] {
			...
			new Animation(4170) };
	private static final GameObject CATAPULT = new GameObject(15616, 10, 0,
			2840, 3548, 1);

	/**
	 * Shotput Activity (Strength)
	 */
	private static final Tile SHOTPUT_FACE_18LB = new Tile(2876, 3549, 1),
			SHOTPUT_FACE_22LB = new Tile(2876, 3543, 1);
	...
	private static final int[] DEFENDERS = { 20072, 8850, 8849, 8848, 8847,
			8846, 8845, 8844 };
	public static final Tile CYCLOPS_LOBBY = new Tile(2843, 3535, 2);
				...
				EngineTaskManager.schedule(new EngineTask() {
					...
					public void run() {
						if (defensiveStyle == timer.projectileType) {
							...
							player.applyHit(new Hit(player, Utilities.random(
									10, 50), HitLook.REGULAR_DAMAGE));
						}
					}
				}, 7);
			}
	...
	public boolean processObjectClick1(GameObject object) {
		if (object.getId() >= 15624 && object.getId() <= 15630) {
			...
			if (player.getTemporaryAttributtes().get("thrown_delay") != null) {
				int random = Utilities.getRandom(3);
			...
			if (Engine.isSpawnedObject(object))
				return false;
			player.lock(2);
			GameObject opened = new GameObject(object.getId(),
	...
	@Override
	public boolean handleItemOnObject(final GameObject object, Item item) {
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		if (interfaceId == 411) {
			...
			if (componentId == 44) {
				...
				for (int i = 0; i < 2; i++) {
					GameObject object = new GameObject(i == 1 ? 66599 : 66601,
							0, 2, 2846, 3535, 2);
					if (Engine.isSpawnedObject(object))
						return false;
					player.lock(2);
	...
	public void prepareShotput(final byte stage, final boolean is18LB) {
		...
		EngineTaskManager.schedule(new EngineTask() {
			...
			public void run() {
				ticks++;
				int distance = Utilities.random(1, (player.getSkills()
						.getLevel(Skills.STRENGTH) / 10) + (is18LB ? 5 : 3));
				if (ticks == 3) {
					Tile tile = new Tile(player.getX() + distance, player
							.getY(), 1);
					Engine.sendProjectile(player, tile, 690, 50, 0, 30, 16, 15,
							0);
					...
					player.setWarriorPoints(STRENGTH,
							base + Utilities.random(2));
	...
	private void balanceKeg(final GameObject object) {
		...
		EngineTaskManager.schedule(new EngineTask() {
			...
			public void run() {
				if (kegCount == 0)
					player.getGlobalPlayerUpdate().setRenderEmote(2671);
				kegCount++;
	...
	private void loseBalance() {
		...
		player.lock(2);
		player.applyHit(new Hit(null, Utilities.random(20, 40),
