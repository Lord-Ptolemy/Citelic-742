...
import com.citelic.game.entity.npc.impl.others.CastleWarBarricade;
...
import com.citelic.game.entity.player.content.dialogue.impl.objects.CastleWarsScoreboard;
...
public final class CastleWars {
	private static class PlayingGame extends TimerTask {
		...
		public void dropFlag(Tile tile, int flagTeam) {
			...
			GameObject flagDroped = new GameObject(flagTeam == SARADOMIN ? 4900
					: 4901, 10, 0, tile.getX(), tile.getY(), tile.getZ());
			spawnedObjects.add(flagDroped);
			Engine.spawnObject(flagDroped, true);
		...
		private void makeSafe(int flagTeam) {
			...
			if (flagStand == null)
				return;
			Engine.destroySpawnedObject(flagStand, false);
		...
		public void reset() {
			...
			for (GameObject object : spawnedObjects)
				Engine.destroySpawnedObject(object, object.getId() == 4377
		...
		public void takeFlag(Player player, int team, int flagTeam,
			...
			if (!droped) {
				...
				Engine.spawnObject(flagStand, false);
			} else {
				spawnedObjects.remove(object);
				Engine.destroySpawnedObject(object, true);
				if (flagTeam == team) {
					makeSafe(flagTeam);
					return;
				}
	...
	public static final Tile LOBBY = new Tile(2442, 3090, 0),
			SARA_WAITING = new Tile(2381, 9489, 0), ZAMO_WAITING = new Tile(
					2421, 9523, 0), SARA_BASE = new Tile(2426, 3076, 1),
	...
	public static void joinPlayingGame(Player player, int team) {
		...
		player.setNextTile(new Tile(team == ZAMORAK ? ZAMO_BASE : SARA_BASE, 1));
	}
	...
	public static void joinPortal(Player player, int team) {
		...
		if (player.getFamiliar() != null && isAllowed(player) == false) {
			player.getPackets()
					.sendGameMessage(
							"You cannot bring a beast of burden familiar into the arean.");
			return;
		}
		for (Item item : player.getInventory().getItems().getItems()) {
			...
			if (item.getId() == 12093 || item.getId() == 12031
					|| item.getId() == 12031) {
				player.getPackets()
						.sendGameMessage(
								"You cannot bring beast of burden pouches into the arena.");
				return;
			}
		}
		...
		player.setNextTile(new Tile(team == ZAMORAK ? ZAMO_WAITING
				: SARA_WAITING, 1));
		player.getMusicsManager().playMusic(318);
		if (playingGame == null && waiting[team].size() >= 6)
			// 9players to
			// start
			createPlayingGame();
		else
	...
	public static void removePlayingPlayer(Player player, int team) {
		...
		player.setNextTile(new Tile(LOBBY, 2));
		...
		for (int i = 0; i < waiting.length; i++) {
			for (Player player : waiting[i].toArray(new Player[waiting[i]
				...
				if (player.getFamiliar() != null && isAllowed(player) == false) {
					...
					player.getPackets()
							.sendGameMessage(
									"You cannot bring a beast of burden familiar into the arena.");
				}
			}
		}
	}
