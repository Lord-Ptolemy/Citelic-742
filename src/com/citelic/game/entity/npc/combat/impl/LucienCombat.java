...
public class LucienCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utilities.getRandom(10) == 0) {
			...
			for (Entity t : possibleTargets) {
				...
				if (!tiles.containsKey(t.getX() + "_" + t.getY())) {
					...
					Engine.sendProjectile(npc,
							new Tile(t.getX(), t.getY(), npc.getZ()), 1900, 34,
							0, 30, 35, 16, 0);
				}
			}
			EngineTaskManager.schedule(new EngineTask() {
				...
				public void run() {
					...
					for (int[] tile : tiles.values()) {
						...
						Engine.sendGraphics(null, new Graphics(1896), new Tile(
								tile[0], tile[1], 0));
						for (Entity t : possibleTargets)
		...
		} else if (Utilities.getRandom(10) == 0) {
		...
		if (attackStyle == 0) { // normal mage move
			...
			Engine.sendProjectile(npc, target, 2963, 34, 16, 40, 35, 16, 0);
			...
			Engine.sendProjectile(npc, target, 1904, 34, 16, 30, 35, 16, 0);
			...
			Engine.sendProjectile(npc, target, 1899, 34, 16, 30, 95, 16, 0);
			delayHit(
					npc,
					4,
					target,
				...
				public void run() {
					...
					for (int dir = 0; dir < dirs[0].length; dir++) {
						final Tile tile = new Tile(new Tile(target.getX()
								+ dirs[0][dir], target.getY() + dirs[1][dir],
								target.getZ()));
						if (Engine.canMoveNPC(tile.getZ(), tile.getX(),
								tile.getY(), size)) { // if
							// found
							// done
