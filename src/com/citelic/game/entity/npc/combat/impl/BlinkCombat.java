...
public class BlinkCombat extends CombatScript {
	...
	private void attackSounds(NPC npc) {
		...
		case 1:
			...
			break;
		case 2:
			npc.setNextForceTalk(new ForceTalk("A whole new Engine!"));
			...
			public void run() {
				...
				for (Entity t : npc.getPossibleTargets()) {
					if (!t.withinDistance(npc, 3))
						continue;
					delayHit(npc, 1, t, new Hit(npc, Utilities.random(100, 850),
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utilities.random(15) <= 5) // playing his random sounds
			attackSounds(npc);
		if (!specialRapidMagic && Utilities.random(15) == 2 && !sprayAttack
				...
				public void run() {
					...
					for (Entity t : npc.getPossibleTargets()) {
						if (!t.withinDistance(npc, 10))
							continue;
						delayHit(npc, 1, t, new Hit(npc, Utilities.random(150),
								HitLook.MAGIC_DAMAGE));
						t.setNextGraphics(new Graphics(2855, 0, 0));
					}
		...
		} else if (!sprayAttack && Utilities.random(25) <= 2)
			sprayAttack(npc, target);

		/*
		 * / npc.setNextForceTalk(new ForceTalk( "H..here it comes!"));
		 * npc.playSound(2989, 2); EngineTasksManager.schedule(new EngineTask() {
		 * 
		...
		if (attackStyle == 1 && !specialRapidMagic && !sprayAttack) { // range
			...
			Engine.sendProjectile(npc, target, 2853, 18, 18, 50, 50, 0, 0);
			for (Entity t : npc.getPossibleTargets()) {
				if (!t.withinDistance(npc, 11))
					continue;
				Engine.sendProjectile(target, t, 2853, 18, 18, 50, 50, 0, 0);
				delayHit(
						npc,
						1,
						t,
