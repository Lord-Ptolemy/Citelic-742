...
public class NomadCombat extends CombatScript {
						...
						public void run() {
							if (!secondLoop) {
								...
								if (npc.clipedProjectile(target, false)) {
									...
									Engine.sendProjectile(npc, target, 1658,
											30, 30, 75, 25, 0, 0);
								}
					...
					EngineTaskManager.schedule(new EngineTask() {
						...
						public void run() {
							if (!secondLoop) {
								...
								player.setNextFaceTile(new Tile(player.getX(),
										player.getY() + 1, 0));
								...
								Engine.sendProjectile(npc, target, 2280, 30,
										30, 5, 25, 0, 0);
								nomad.setCantFollowUnderCombat(false);
								nomad.setNextMovePerform();
								stop();
	...
	private void spawnFlameVortex(Tile tile) {
		if (!Engine.isNotCliped(tile.getZ(), tile.getX(), tile.getY(), 1))
			return;
