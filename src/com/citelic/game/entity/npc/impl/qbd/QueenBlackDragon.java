...
public final class QueenBlackDragon extends NPC {
	...
	public void openRewardChest(boolean replace) {
		...
		if (replace) {
			Engine.spawnObject(
					new GameObject(70817, 10, 0, base.transform(30, 28, -1)),
					true);
		}
	}
	...
	private void prepareRewards() {
		...
		rewards.add(new Item(24372, 1 + Utilities.random(6)));
		rewards.add(new Item(24336, 50 + Utilities.random(51)));
		...
		for (int[] reward : REWARDS) {
			Item item = new Item(reward[0], reward[1]
					+ Utilities.random(reward[2] - reward[1]));
			for (int i = 0; i < reward[3]; i++) {
				rewardTable.add(item);
			}
		}
		Collections.shuffle(rewardTable);
		for (int i = 0; i < 1 + Utilities.random(3); i++) {
			rewards.add(rewardTable.get(Utilities.random(rewardTable.size())));
		}
	}
	...
	public void processNPC() {
		...
		if (ticks == -20) {
			...
			Engine.removeObject(
					new GameObject(70778, 10, 0, base.transform(33, 31, 0)),
					true);
			Engine.spawnObject(
					new GameObject(70790, 10, 0, base.transform(31, 29, 0)),
					true);
			Engine.spawnObject(
					new GameObject(70775, 10, 0, base.transform(31, 29, -1)),
					true);
		} else if (ticks == -1) {
			return;
		}
		...
		if (spawningWorms) {
			...
			QueenAttack attack;
			while (!(attack = attacks[Utilities.random(attacks.length)])
	...
	public void sendDeath(Entity source) {
		...
		case 1:
			...
			activeArtifact = new GameObject(70777, 10, 0, base.transform(33,
					31, 0));
			attacker.getPackets()
					.sendGameMessage(
		...
		case 2:
			...
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70844, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new GameObject(70780, 10, 0, base.transform(24,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
		...
		case 3:
			...
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70846, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new GameObject(70783, 10, 0, base.transform(42,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
		...
		case 4:
			...
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70848, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new GameObject(70786, 10, 0, base.transform(33,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
		...
		Engine.spawnObject(activeArtifact, true);
		setCantInteract(true);
		if (phase < 5) {
	...
	public void setPhase(int phase) {
		...
		case 5:
			...
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70837, 10, 0, base.transform(22, 24, -1)));
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70840, 10, 0, base.transform(34, 24, -1)));
			attacker.getPackets()
					.sendGameMessage(
	...
	public void spawnWorm() {
		...
		final Tile destination = base.transform(28 + Utilities.random(12),
				28 + Utilities.random(6), 0);
	...
	public void switchState(QueenState state) {
		...
		case DEFAULT:
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70822, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70818, 10, 0, base.transform(39, 35, -1)));
			break;
		case HARDEN:
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70824, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70820, 10, 0, base.transform(39, 35, -1)));
			break;
		case CRYSTAL_ARMOUR:
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70823, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new GameObject(70819, 10, 0, base.transform(39, 35, -1)));
			break;
		default:
			break;
		}
