...
public class KreeArra extends NPC {
	...
	public void sendDeath(Entity source) {
		...
		EngineTaskManager.schedule(new EngineTask() {
			...
			public void run() {
				if (loop == 0) {
					KreeArra.this.setNextAnimation(new Animation(defs
							.getDeathEmote()));
				} else if (loop >= defs.getDeathDelay()) {
					KreeArra.this.drop();
					KreeArra.this.reset();
					KreeArra.this.setLocation(KreeArra.this.getRespawnTile());
					KreeArra.this.finish();
					KreeArra.this.setRespawnTask();
					stop();
				}
				loop++;
			}
	...
	public void setRespawnTask() {
		...
		CoresManager.slowExecutor.schedule(new Runnable() {
			@Override
			public void run() {
				KreeArra.this.setFinished(false);
				Engine.addNPC(npc);
				npc.setLastRegionId(0);
				Engine.updateEntityRegion(npc);
				KreeArra.this.loadMapRegions();
				KreeArra.this.checkMultiArea();
