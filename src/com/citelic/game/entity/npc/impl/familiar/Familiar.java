...
public abstract class Familiar extends NPC implements Serializable {
	...
	public static void confirmLeftOption(Player player) {
		...
		player.getPackets().closeInterface(res ? 162 : 187);
	}
	...
	public boolean titanAttack = false;

	public boolean titanSpec = false;
	...
	public void call() {
		if (getAttackedBy() != null
				&& getAttackedByDelay() > Utilities.currentTimeMillis()) {
	...
	public void call(boolean login) {
		...
		if (login) {
			...
			checkNearDirs = Utilities.getCoordOffsetsNear(size);
			sendMainConfigs();
		} else
			removeTarget();
		Tile teleTile = null;
		for (int dir = 0; dir < checkNearDirs[0].length; dir++) {
			...
			if (Engine.canMoveNPC(tile.getZ(), tile.getX(), tile.getY(), size)) { // if
				// found
				// done
				teleTile = tile;
				break;
	...
	public void processNPC() {
		...
		if (!getCombat().process()) {
			if (isAgressive()
					&& owner.getAttackedBy() != null
					&& owner.getAttackedByDelay() > Utilities
							.currentTimeMillis()
					&& canAttack(owner.getAttackedBy())
					&& Utilities.getRandom(25) == 0)
	...
	public void sendDeath(Entity source) {
		...
		owner.getPackets().sendGlobalConfig(168, 4); // resets to
		// inventory tab,
		// cbf being in a
		// glitched
		// summoning tab
