...
public class Region {
			...
			while ((incr = landStream.readSmart2()) != 0) {
				...
				while ((incr2 = landStream.readUnsignedSmart()) != 0) {
					...
					if (objectPlane < 0 || objectPlane >= 4 || plane < 0
							|| plane >= 4)
						continue;
					addObject(new GameObject(objectId, type, rotation, localX
							+ regionX, localY + regionY, objectPlane),
							objectPlane, localX, localY);
				}
			}
		}
		if (GameConstants.DEBUG && landContainerData == null
				&& landArchiveId != -1
	...
	public void addObject(GameObject object, int plane, int localX, int localY) {
		if (Engine.restrictedTiles != null) {
			for (Tile restrictedTile : Engine.restrictedTiles) {
				if (restrictedTile != null) {
					...
					if (object.getX() == restX && object.getY() == restY
						...
						Engine.spawnObject(
								new GameObject(-1, 10, 2, object.getX(), object
										.getY(), object.getPlane()), false);
						return;
					}
				}
			}
	...
	public List<Integer> getPlayerIndexes() {
		...
		List<Integer> flushedIndices = new ArrayList<Integer>();

		for (Integer playerIndex : playersIndexes) {
			if (flushedIndices.contains(playerIndex))
				continue;

			flushedIndices.add(playerIndex);
		}
	...
	public int getMask(int plane, int localX, int localY) {
		...
		if (localX >= 64 || localY >= 64 || localX < 0 || localY < 0) {
			Tile tile = new Tile(map.getRegionX() + localX, map.getRegionY()
					+ localY, plane);
			int regionId = tile.getRegionId();
			int newRegionX = (regionId >> 8) * 64;
			int newRegionY = (regionId & 0xff) * 64;
			return Engine.getRegion(tile.getRegionId()).getMask(plane,
	...
	public void setMask(int plane, int localX, int localY, int mask) {
		...
		if (localX >= 64 || localY >= 64 || localX < 0 || localY < 0) {
			...
			int newRegionX = (regionId >> 8) * 64;
			int newRegionY = (regionId & 0xff) * 64;
			Engine.getRegion(tile.getRegionId()).setMask(plane,
	...
	public FloorItem getGroundItem(int id, Tile tile, Player player) {
		if (floorItems == null)
			return null;
